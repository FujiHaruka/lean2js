...
/-!
# The semantics of the generated JavaScript, held inside Lean
...
def helper (name : String) (args : List JsValue) : Option JsResult :=
  ...
  | "__abs", [.num a] => some (.ok (.num (if a < 0 then -a else a)))
  | "__abs", [.bigint a] => some (.ok (.bigint (if a < 0 then -a else a)))
  | "__min", [.num a, .num b] => some (.ok (.num (if a <= b then a else b)))
  | "__min", [.bigint a, .bigint b] => some (.ok (.bigint (if a <= b then a else b)))
  | "__max", [.num a, .num b] => some (.ok (.num (if a <= b then b else a)))
  | "__max", [.bigint a, .bigint b] => some (.ok (.bigint (if a <= b then b else a)))
