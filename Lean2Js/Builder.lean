...
namespace Lean2Js.Core.Builder
...
def range' (e : Expr) : Expr := .un .range e
...
def sortByKey' (arr : Expr) (binder : String) (body : Expr) : Expr := .sortByKeyE arr binder body
...
def dict (value : Ty) (entries : List (String × Expr)) : Expr := .dictLit value false entries
