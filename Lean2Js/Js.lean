...
def declareFunc (d : Core.Decl) (codes : List String) (doc : Option String := none) : String :=
  ...
  let quoted := codes.map fun c => "`" ++ c ++ "`"
  let listed := match quoted.reverse with
    | [] => ""
    | [only] => only
    | last :: front => String.intercalate ", " front.reverse ++ " or " ++ last
  let throws :=
    if codes.isEmpty then []
    else [" * @throws {Error} whose `code` is " ++ listed ++ "."]
