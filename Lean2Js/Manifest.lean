...
def compilerVersion : String := "0.4.0"
...
belong together; without it a package that was edited after it was written reads exactly like one that
was not. The manifest cannot name its own digest, which is why the file that carries the list is the one
file not in it. -/
def Artifact.toJson (a : Artifact) (artifacts : List (String × String)) : Json :=
  ...
  .obj [
    ...
    ("source", .str a.source),
    ("artifacts", .arr (artifacts.map fun (file, sha256) =>
      Json.obj [("file", .str file), ("sha256", .str sha256)])),
    ("exports", .arr exports),
    ("theorems", .arr claims),
...
def Artifact.toReadme (a : Artifact) : String :=
  ...
  let integrity :=
    [ "## Checking this is the package that was built",
      "",
      "`proof-manifest.json` names the SHA-256 of every other file here. What the theorems above are \
        about is the module those digests name, so a file that does not match one is not the file the \
        claims beside it speak about.",
      "",
      "```sh",
  ...
  String.intercalate "\n" (
    [ s!"# {a.manifest.package}",
      ...
      "" ] ++ usage ++
    [ "## Exports",
      "" ] ++ exports ++ [ "" ] ++ errors ++ proofs ++ integrity)
