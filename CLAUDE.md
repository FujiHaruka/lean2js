...
ファイルを書くので、生成したベクタ全件について `eval` と JS の模型、`eval` と small-step、`eval` と
Node で動かした生成物が一致しなければ書き出し自体が失敗する。落ちたときに直すのはコンパイラか意味論であって、検査の側ではない。
...
- **日本語なのは `README.md` / `docs/` / `templates/**/*.md` / この `CLAUDE.md` だけ。**
