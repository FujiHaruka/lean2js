...
**`.d.ts` の型が通ることは、入口検査を通ることを意味しない。厳しいのは 1 点だけ。** `Int53` と
`UInt32` はどちらも `number` に写るので、範囲を外れた数も TS では通り、渡せば `typeError` になる。
オブジェクトのフィールドは名前で読むので、並びは縛らない。宣言に無いキーが載っていても通り、本体に
届く前に落ちる。
