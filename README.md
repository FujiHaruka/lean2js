...
```
Lean のリファレンス意味論  ──証明（式の断片・関数 43 本／入口 67 本）──  生成した JS
        │                                        │
        │                       └──実行時検査（成果物の全ベクタ）──┘
        │
...
- **証明（式）**: `Core.Expr` の 35 形のうち **27 形**について、`eval` が値を返すなら生成コードも同じ値を
  返す。リテラル・変数・条件式・`let` 束縛・単項演算・**二項演算のすべて**（算術・比較・等値・論理・
  連結の 16 演算子）に加えて、**部分式を評価して値を組み立てるだけの形**すべて —— `Option` と `Result`
  の構成、構造体の構成と射影、配列リテラルと添字・`slice` / `reverse`・長さ、辞書リテラルと
  `get` / `has` / `set` / `keys` / `values` / `delete`、文字列の `trim` / 大文字小文字 /
  `startsWith` / `endsWith` / `includes` / `split` / `substring`。外れる 8 形は、呼び出しと関数参照
  （他の宣言の本体へ行く）と、`match` および配列走査 5 つ（束縛を変えながら部分式を評価する）
- **証明（関数・返す側）**: 本体がその 27 形に収まる**公開関数 43 本**（全 67 本のうち）については、主張が
  呼び出しの単位で立つ。**どんな引数でも**、`eval` が値を返すなら生成モジュールの同名関数が同じ値を返す。
  入口の型検査、`__pN` から宣言名への引き渡し、先頭の `let` が `const` 文に開かれるところまで含む。残り
  24 本を受け持っているのは下の実行時検査で、そちらは「試した入力については合っていた」の形をしている
- **証明（関数・落ちる側）**: 同じ **43 本**について、`eval` が値を返さずに落ちるなら、生成モジュールの
  同名関数は**同じコード**で落ちる。断片が届く落ち方はゼロ除算・`Int53` 溢れ・範囲外アクセス
  （添字・`slice` / `substring`）の 3 つで、型エラーには届かないことを型の健全性が示す。除くのは
  `eval` 側の燃料切れだけ —— 燃料は `eval` を全域にするための装置で、生成コードについての主張は
  「十分な燃料で」の形をしているから、写す相手がいない
- **型の健全性**: コンパイラが型 `T` と判断した式を `eval` が評価して値が返るなら、その値は `T` を
  満たす。上の 27 形すべて（二項演算は演算子を問わず）について証明済みで、生成コードが被演算子の型で
  分岐する箇所を閉じているのがこれ
- **証明（関数・弾く側）**: 公開関数は本体に入る前に、引数を宣言した型と構造的に照合する。**公開関数
  ...
  を投げる。本体の形を見ない主張なので、返す側の 43 本より広く、断片の外の関数にも立つ。一致の主張に
  「宣言した型を満たす引数について」という但し書きが要らないのはこれによる。落ちること自体は両側で
