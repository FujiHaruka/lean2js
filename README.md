...
| Strings (`Str.trim` / `Str.upper` / `Str.lower` / `Str.startsWith` / `Str.endsWith` / `Str.includes` / `Str.split` / `Str.substring`) and an `Int53` in decimal (`Int53.toString`) | Regular expressions |
