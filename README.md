...
```
Lean のリファレンス意味論  ──証明（式の断片・関数 58 本／入口 67 本）──  生成した JS
        │                                        │
        │                       └──実行時検査（成果物の全ベクタ）──┘
        │
...
- **証明（式）**: `Core.Expr` の 35 形のうち **33 形**について、`eval` が値を返すなら生成コードも同じ値を
  ...
  `startsWith` / `endsWith` / `includes` / `split` / `substring` ——、そして**束縛を変えて部分式を
  評価する形** —— `match` と配列走査 5 つ（`map` / `filter` / `find` / `all`・`any` / `reduce`）。
  外れる 2 形は呼び出しと関数参照で、そこだけが他の宣言の本体へ行く
- **証明（関数・返す側）**: 本体がその 33 形に収まる**公開関数 58 本**（全 67 本のうち）については、主張が
  呼び出しの単位で立つ。**どんな引数でも**、`eval` が値を返すなら生成モジュールの同名関数が同じ値を返す。
  入口の型検査、`__pN` から宣言名への引き渡し、先頭の `let` が `const` 文に開かれるところまで含む。残り
  9 本はどれも呼び出しを通る関数で、受け持っているのは下の実行時検査。そちらは「試した入力については
  合っていた」の形をしている
- **証明（関数・落ちる側）**: 同じ **58 本**について、`eval` が値を返さずに落ちるなら、生成モジュールの
...
- **型の健全性**: コンパイラが型 `T` と判断した式を `eval` が評価して値が返るなら、その値は `T` を
  満たす。上の 33 形すべて（二項演算は演算子を問わず）について証明済みで、生成コードが被演算子の型で
  分岐する箇所を閉じているのがこれ
- **証明（関数・弾く側）**: 公開関数は本体に入る前に、引数を宣言した型と構造的に照合する。**公開関数
  ...
  を投げる。本体の形を見ない主張なので、返す側の 58 本より広く、断片の外の関数にも立つ。一致の主張に
