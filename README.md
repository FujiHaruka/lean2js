...
| `String` 操作（`trim` / 大文字小文字 / `startsWith` / `includes` / `split` / `substring`） | 正規表現 |
| `Dict`（文字列キー、`Map` として出力） | 素のオブジェクトを辞書として使うこと |
...
- **停止性**: 関数は自分より前に宣言された関数しか呼べないので、自己再帰も相互再帰もコンパイルが通らない。
  ...
  が受け持つ
- **実行時検査**: 出荷する成果物の全ベクタ（現在 18665 件）について、`eval` と JS の模型、および
