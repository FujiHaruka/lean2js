...
```
your Lean def  ──certificate (one per declaration, all 101 shipped)──  reference semantics
      │
      └──your theorems are ordinary Lean equations about this def

reference semantics  ──proof (every expression form, all 100 public functions)──  generated JS
      │                                        │
...
- **What the proofs reach** — **all 35 forms** of `Core.Expr` and **all 100 public functions**. Where
  ...
  ceiling of 10000 the artifact runs at, this example needs 713).
...
- **The `.d.ts`** — an argument the entry check accepts satisfies the `.d.ts` type
  ...
  trusted here.
- **Outside the proofs** — every vector generated for the artifact (36761 of them for this example) is
...
- **The list not drifting from the proofs** — the theorems in the manifest are not written by hand.
  ...
  are all there rather than 101 restatements of one shape. That no theorem rests on an axiom beyond
