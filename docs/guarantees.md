...
```
利用者の Lean の def  ──証明書（宣言ごと 1 本・出荷する 69 本すべて）──  リファレンス意味論
        │
        └──利用者の定理は、この def についての普通の Lean の等式

Lean のリファレンス意味論  ──証明（式のすべての形・公開関数 68 本すべて）──  生成した JS
        │                                        │
        │                       └──実行時検査（成果物の全ベクタ）──┘
        │
...
- **証明が届く範囲** — `Core.Expr` の **35 形すべて**と、公開関数 **68 本すべて**。`eval` が値を返すなら
  ...
  値を返したこと自体から出る。落ちる側は引数の個数と宣言した型を仮定し、入口で弾く側は公開宣言で
  あることと、同じキーを 2 度持つ辞書（`Map` が持てない形）を除くことを仮定する。`eval` 側の燃料切れは
  どの向きでも答えに入らない（[`cost`] が必要な燃料の上界を構文から計算し、
  [`progOk`] が呼び出しが前へしか進まないことを見る。出荷時の上限 10000 に対してこの例題は 489）
...
- **証明の外** — 成果物ごとに生成する全ベクタ（この例題で 25075 件）を、`lean2js` が書き出す前に
...
- **一覧が証明とずれないこと** — manifest に載る定理は手で書かない。`lean2js` が同じ名前空間の公開定理を
  ...
  69 通りの言い換えではない。どの定理も `propext` / `Classical.choice` /
...
届く前に落ちる。ただし**その綴りで呼んだときに同じ値が返ることを受け持つのは、証明ではなく実行時検査**。
一致と trap の主張は引数を正規の綴り（`encodeValue`）で書いたときのもので、入口検査が通す別の綴りは
`shapes` としてベクタに載り、Lean の模型と Node の両方で突き合わせられる。
