...
```
your Lean def  ──certificate (one per declaration, all 99 shipped)──  reference semantics
      │
      └──your theorems are ordinary Lean equations about this def

reference semantics  ──proof (every expression form, all 98 public functions)──  generated JS
      │                                        │
...
- **What the proofs reach** — **all 35 forms** of `Core.Expr` and **all 98 public functions**. Where
  ...
  ceiling of 10000 the artifact runs at, this example needs 699).
...
- **The `.d.ts`** — an argument the entry check accepts satisfies the `.d.ts` type
  ...
  trusted here.
- **Outside the proofs** — every vector generated for the artifact (35924 of them for this example) is
...
- **The list not drifting from the proofs** — the theorems in the manifest are not written by hand.
  ...
  are all there rather than 99 restatements of one shape. That no theorem rests on an axiom beyond
