...
```
your Lean def  ──certificate (one per declaration, all 93 shipped)──  reference semantics
      │
      └──your theorems are ordinary Lean equations about this def

reference semantics  ──proof (every expression form, all 92 public functions)──  generated JS
      │                                        │
...
- **What the proofs reach** — **all 35 forms** of `Core.Expr` and **all 92 public functions**. Where
  ...
  ceiling of 10000 the artifact runs at, this example needs 657).
...
- **The artifact itself** — the `index.js` that is written **reads back as the same AST**
  ([`parseModule_render_of_compileProgram`], no caveat). The 50 run-time helpers the generated code
  calls under the `__` prefix are hand-written, but **everything the model assumes of them** agrees with
  what the printer writes out: every one of the 36 rows of the table ([`helpers_ship_as_modelled`]), and
...
- **The `.d.ts`** — an argument the entry check accepts satisfies the `.d.ts` type
  ...
  trusted here.
- **Outside the proofs** — every vector generated for the artifact (32270 of them for this example) is
...
- **The list not drifting from the proofs** — the theorems in the manifest are not written by hand.
  ...
  are all there rather than 93 restatements of one shape. That no theorem rests on an axiom beyond
