...
足すための計画。**着手はまだ先で、ここに書くのは何を足すかと、その値段。**

## 文脈

今の到達点は測った数字で言える。公開関数 68 本、manifest の定理 25 本、出荷前に照合する差分ベクタ
25075 件、`Core.Expr` は 35 形。実行時ヘルパは 49 本で、模型がヘルパについて仮定している表 35 行の
...
- **案 A: 呼び出し位置に展開する。** 印を付けた `def` を reify が展開し、既存の形へ落とす。
  生成 JS に関数が増えない代わりに、式が深くなって燃料を食い、生成コードが読みにくくなる。
- **案 B: 出荷はするが公開境界に出さない。** 印を付けた `def` を宣言として読み出し、`index.js` には
  関数として出すが `.d.ts` と輸出には出さない。呼び出しは普通の `call` のまま。

**案 B を採る。** 仕組みはもう半分ある —— 関数を引数に取る宣言はすでに「出るが公開されない」扱いで、
`Compile` の `exported := d.isPublic` がその 1 行。これを型から決まる `isPublic` と、印から決まる
「輸出するか」に割る。燃料も増えず、生成コードは呼び出しのまま読める。**案 A へ倒れる条件は、
ライブラリの名前空間をまたいで宣言を集める仕事（`Gather`）が、展開より高くついたとき。**
...
## Step 1. 出荷しない宣言（層 0 の機構）

**書けるようになること**: 補助の `def` を、公開境界に出さずに呼べる。

**形**: Approach の案 B。`Decl` に輸出するかの印を持たせ、`Compile` の `exported` をそこから引く。
`Gather` がライブラリの名前空間から印の付いた宣言を集め、`callsPrecede` の順に並べる。

**印を付けられるのは非再帰で、本体が部分集合の中だけで書かれた `def`。** `Arr.take xs n` を
`Arr.slice xs 0 n` と書くのはよいが、`List.take` の再帰をそのまま印に乗せることはできない。

**触る場所**: `Core`（`Decl` の印）、`Compile`（`exported`）、`Gather`（名前空間をまたいで集める）、
`Dts`（輸出しない宣言が `.d.ts` に出ないこと）、`Decl.lean`（`decl_refuses` は公開宣言についての主張
なので、輸出しない宣言が入口検査を持たないことを言う）。

**保証の境界**: 変わらない。ただし**輸出しない宣言はベクタが作られない**（ベクタは公開関数ごと）。
...
**触る場所**: `Prelude.lean`（語彙）、`Example.lean`（語彙ごとに公開宣言を 1 本）、`SYNTAX.md`。
`Reify.lean` は Step 1 の機構が効くので動かない。
...
- `docs/guarantees.md` — ベクタ件数（今 25075 件）、実行時ヘルパの本数と仮定の表の行数
- `docs/next-milestone-plan.md` — 「言語が凍っている」を数えている行。Step 1 に着手した時点で、
  凍結をどの範囲で解いたかを書く
