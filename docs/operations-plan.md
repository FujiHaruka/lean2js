...
今の到達点は測った数字で言える。公開関数 107 本、manifest の定理 28 本、出荷前に照合する差分ベクタ
38298 件、`Core.Expr` は 36 形。実行時ヘルパは 60 本で、模型がヘルパについて仮定している表 40 行の
...
無しで全形を覆っている。`Core.Expr` に形を 1 つ足すのは、もはや「`Agree` の全件検査に任せる
...
- **`helper_strBin` の持ち上げは済んでいる**（2026-09-19, `6e0d0e1`）。`applyStrBin op (.str a) (.str b)`
...
- `docs/guarantees.md` — ベクタ件数（今 36983 件）、実行時ヘルパの本数と仮定の表の行数
...
- **`strBin` の引数型を開いた**（2026-09-19, `6e0d0e1`） — `Compile.strBinArgTys : StrBinOp → Ty × Ty` を
...
- **Step 4 の 4 つめ**（2026-09-19, `6802d7f`） — `Str.repeat` が入った。**3 つの道のうち、形を 1 つも
  ...
  `n <= 0 ? "" : (n % 2 === 1 ? s : "") + __rep(s + s, trunc(n / 2))` の半分割で書いた。深さは
  `log2 n` なので実機のスタックにも届かない。**`prim` 12 種・メソッド 13 種のままで TCB は増えて
  いない**し、`Helper.Stmt` も `Core.Expr` も 1 形も動いていない。
  証明は `n.toNat` に対する強い帰納法（`calls_rep_aux`、燃料は `f + 4 * n.toNat + 16`）と、
  `strRepeatFrom (s ++ s) m = strRepeatFrom s (2 * m)` の 1 本だけで閉じた。
  **上限は積で述べ、検査は商で書く。** `applyStrBin` は `|s| * n` が `int53Max` を超えたら
  `int53Overflow`、`__repeat` は `n > __i53div(int53Max, |s|)` で同じ所に落ちる。両者が同値なのは
  `div_bound`。**`Math.trunc(int53Max / len)` では書けない** —— `__i53div` の docstring が言うとおり
  `a` が 2^53 の近くだと浮動小数点の商が 1 ずれるので、BigInt で割る既存のヘルパを呼ぶ。
  **`n` が 0 以下は trap せず `""`** —— `Str.split s ""` と同じで、答えが定義されているぶん狭い。
  空文字列は長さを見る前に `""` を返す（どんな回数でも伸びないので、上限を聞く意味が無い）。
  実行時ヘルパは `__repeat` と `__rep` で 54 → 56 本、模型の表は 39 → 40 行。
  `Example.lean` に `receiptRule` を 1 本。**回数はリテラル 32 で書いた** —— ベクタの `Int53` プールは
  `int53Max` や 10^6 を含むので、回数を引数のまま通すと照合が数 GB の文字列を作りに行く。
...
- **Step 4 の 5 つめ**（2026-09-19, `ed69d06`） — `Str.padStart` が入って **Step 4 は終わった。
  ...
  **`Core.Expr` も実行時ヘルパも模型の表も 1 つも動いていない**（35 形・56 本・40 行のまま）。
  **パッドは幅の所で切る**ので、複数文字のパッドが幅を越えない。コードポイントで数えるのは
  `Str.length` と `Str.substring` から来るので、ネイティブの `padStart`（UTF-16 単位）の食い違いは
  最初から無い。`Example.lean` に `amountColumn` を 1 本、幅はリテラル 12。
  公開関数 100 → 101 本、宣言 101 → 102、差分ベクタ 36761 → 36983 件、燃料は 713 → 926
  （`@[expand]` は呼ぶ場所に本体を書き出すので、`padStart` 1 本で 200 以上増える）。
  `SYNTAX.md` の「無い操作」に残ったのは `padEnd` の 1 行だけで、そこには
  `s ++ substring (repeat pad k) 0 k` のレシピを置いた。
- **Step 5**（2026-09-19, 判断のみ。着手しない） — **`sortBy` の非目標は据え置く。覆すための条件は
  Step 4 で 1 つ減った。** 残っている理由は「比較関数が全順序であることの証明義務が利用者に出る」
  ことだけで、`Arr.sortByKey xs key`（`key` は宣言の名前、返りは `Int` か `String`）にすれば
  その 1 つは型から出る。**JS 側にあった「回数で回るループが無い」という障害はもう無い** ——
  `__rep` が示したとおりヘルパ言語は自分を呼べるので、マージソートは `Helper.Stmt` に形を足さずに
  書ける（証明の型紙は `calls_rep_aux` と `calls_eq_aux`）。残る仕事は
  **`Array.prototype.sort` の安定性（ES2019 で仕様化）を模型のどこに置くかと、それが Lean の
  `List.mergeSort` と同じ列を返すことの証明**の 1 本。手書きヘルパでマージソートを書けば、
  ネイティブの `sort` を模型に足さずに済むので、メソッド 13 種も動かない。
  **それでも着手しないのは、値段ではなく順番の問題** —— `docs/next-milestone-plan.md` が数えている
  「式の形が凍っている」は `Core.Expr` 35 形のことで、**`sortByKey` はその 36 形目になる**。
  「既存の形の演算子として足せないか」は測った: `mapE` / `filterE` / `findE` / `reduceE` は
  op を取る 1 つの形ではなく**それぞれ独立した構成子**なので、走査を 1 つ増やすことは
  `strBin` に腕を足すのとは値段が違う。凍結をどの範囲で解くかは、この計画ではなくそちらで決める。
- **Step 5 の続き**（2026-09-19, ユーザーの指示で凍結を開けた） — **`Arr.sortByKey` が 36 形目として
  入った**。見通しどおり、比較関数を取らない形にしたことで利用者の証明義務は消え、手書きのマージ
  ソートにしたことで模型が仮定する JS 組み込みの表は 40 行のまま動かなかった。実行時ヘルパは
  56 → 60 本（`__keyle` / `__merge` / `__msort` / `__sortBy`）、公開関数 105 → 107 本、manifest の
  定理 27 → 28 本、差分ベクタ 37865 → 38298 件、燃料は 962 → 980。**見通しと違ったのは 1 つだけ** ——
  `__merge` を素直な再帰で書くと再帰の深さが要素数になり、数千要素で生成 JS だけが RangeError を
  投げる。ループ 1 本と添字 2 つに書き直して深さを定数に戻した（`docs/sort-plan.md`）。
