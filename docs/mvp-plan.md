...
- compiler correctness — リテラル・変数・条件式・`let` 束縛・単項演算・二項演算について証明済み
  （`Correct.lean`）。二項演算は 16 演算子すべて。算術は `Int53` / `UInt32` / `BigInt` の 3 つの
  被演算子型すべてを覆っていて、`UInt32` の巻き戻しも `>>> 0` / `Math.imul` の模型と突き合わせて
  ある。比較は順序を持つ 4 つの型すべてで、`String` は生成コードの `__strcmp` がコードポイント順に
  並べるところまで含む。等値は「符号化しても等値が保たれる」を型を付けて証明したうえで、スカラーの
  `===` とそれ以外の `__eq` の両方を覆う。断片の外は `Agree` の実行時検査が受け持っている
- 宣言単位の正しさ — 証明済み（`Decl.lean`）。本体が上の 6 形に収まる公開関数 23 本（全 67 本のうち）に
  ついて、`evalCall` が値を返すなら生成モジュールの同名関数の呼び出しが同じ値を返す。入口の `checkTy` が
  ...
  「十分な燃料で」の形で、`callFunctionAt` がその燃料を取る。残り 44 本は `Agree` の全件検査が受け持つ
- 宣言単位の陰性方向 — 証明済み（`Decl.lean`）。公開関数 67 本すべてについて、生成関数の入口を通る引数は
  `eval` も通す引数の符号化である（`decl_refuses`）。裏を返せば `eval` が受け取らない呼び出しは
  `typeError` で落ちる。入口は本体を見ないので `InFragment` が要らず、陽性方向の 23 本より広い。主張は
...
- 本体の陰性方向 — 証明済み（`Correct.lean` / `Decl.lean`）。断片に収まる公開関数 23 本について、
  入口が受け取った引数で `eval` が落ちるなら、生成モジュールの同名関数が**同じコード**で落ちる
  （`fragment_traps_in`、`decl_traps`）。`Agree` が失敗どうしを `Err.code` で比べているので、主張も
  コードの一致にしてある。断片が届く落ち方はゼロ除算と `Int53` 溢れだけで、型エラーには `typeSound` が
  届かせない。`EnvTyped` は「ctx にある名前が env にもある」を言わないので `EnvCovers` を足した。除くのは
  `eval` 側の燃料切れで、生成コード側の主張が「十分な燃料で」の形をしている以上、写す相手がいない
