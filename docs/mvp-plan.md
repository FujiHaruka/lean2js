...
- 宣言単位の陰性方向 — 証明済み（`Decl.lean`）。公開関数 67 本すべてについて、生成関数の入口を通る引数は、
  宣言された型で正規化すると `eval` も通す引数の符号化になる（`decl_refuses`）。裏を返せば `eval` が受け取らない呼び出しは
...
- 本体の陰性方向 — 証明済み（`Correct.lean` / `Decl.lean`）。公開関数 67 本すべてについて、
  ...
  写す相手がいない落ち方は無い —— `eval` 側の燃料切れは `Cost.evalCall_ne_outOfFuel` が排除する
  （`decl_traps_at_cost`）
...
- 燃料 — 証明済み（`Cost.lean`）。`eval` を全域にしている燃料は、この処理系が受け付けるプログラムでは
  尽きない。必要な燃料は式の入れ子の深さと呼び出しの段数だけで決まり（走査はリストの残りを同じ燃料で
  評価する）、呼び出しは前へしか進まないので段数は宣言の本数で押さえられる。`Cost.cost` が上界を
