...
- JS の意味論の模型（`JsSem.lean`）と、出荷する成果物がリファレンス意味論と一致することの実行時検査
  （`Agree.lean`）— 完了
- type soundness — 証明済み（`Sound.lean`）。`Core.Expr` の 35 形のうち 33 形について、コンパイラが型
  `T` と判断した式を `eval` が評価して値が返るなら、その値は `T` を満たす。生成コードは被演算子の型で
  分岐するので、これが無いと `+` が連結か加算かを決められなかった
- compiler correctness — 同じ 33 形について証明済み（`Correct.lean`）。二項演算は 16 演算子すべて。算術は
  ...
  外れる 2 形は呼び出しと関数参照で、そこは `Agree` の実行時検査が受け持っている
- 宣言単位の正しさ — 証明済み（`Decl.lean`）。本体が上の 33 形に収まる公開関数 58 本（全 67 本のうち）に
  ...
  「十分な燃料で」の形で、`callFunctionAt` がその燃料を取る。残り 9 本は `Agree` の全件検査が受け持つ
- 宣言単位の陰性方向 — 証明済み（`Decl.lean`）。公開関数 67 本すべてについて、生成関数の入口を通る引数は
  `eval` も通す引数の符号化である（`decl_refuses`）。裏を返せば `eval` が受け取らない呼び出しは
  `typeError` で落ちる。入口は本体を見ないので `InFragment` が要らず、陽性方向の 58 本より広い。主張は
...
- 本体の陰性方向 — 証明済み（`Correct.lean` / `Decl.lean`）。断片に収まる公開関数 58 本について、
  ...
  写す相手がいない落ち方は `Correct.Mirrorable` が名指ししている 2 つ —— `eval` 側の燃料切れ（生成コード側の
  主張が「十分な燃料で」の形をしている）と、`match` がどの腕にも当たらないとき（生成した連鎖は網羅性検査を
  信じて最後の腕をテストなしで取るので、写すには網羅性検査の正しさが要り、その検査は `partial`）
